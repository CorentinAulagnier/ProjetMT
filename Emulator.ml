...
module Split =
  (struct
    ...
    let encode (t : translator) : translator =
(*récuperation des sauvegarde*)
      let alpha : alphabet =  Alphabet.t
      let s : symbols = symbols.alpha
      let b : int = symbol_size_in_bits.alpha

(*Reecriture de la bande*)   
      let elt (l,ecr,fin) = (*elt translator ecrire find*)
        match t with
          | []-> []
          | e::[]-> ecrire( find(e, s), b)
          | e::su-> ecrire( find(e, s), b)@ elt(su);;
      ;;

(*Codage binaire de l'entier I sur B bits*)
    let ecrire (i : int, b : int) : int list =
      match (i,b) with
        | (0,1) -> [0]
        | (0,_) -> ecrire(i, b-1) :: 0
        | P(i) -> ecrire(int_of_float(i / 2), b-1) :: 1
        | _ -> ecrire(int_of_float(i / 2), b-1) :: 0;;

(*Renvoi si l'entier est Pair ou non*)
    let P (i : int) : bool =
      match i with
        | 0 -> true
        | 1 -> false
...
(*Recherche du numero du symbol*)
    let find (e, alpha : symbol*symbols) : int =
      match alpha with
        | e2::s2 when e=e2 -> 0
        | e2::s2 -> 1 + find(t,s2);;
